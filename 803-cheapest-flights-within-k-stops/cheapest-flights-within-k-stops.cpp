...
public:
    ...
    int findPrice(unordered_map<int, unordered_map<int, int>>& graph, int src, int& dst, int k, vector<vector<int>>& dp)
    {
        if (src==dst) return 0;
        ...
        for (auto& nei : graph[src])
        {
            ...
            int p=findPrice(graph, nei.first, dst, k-1, dp);
    ...
    int findCheapestPrice(int n, vector<vector<int>>& flights, int src, int dst, int k)
    {
        ...
        int ans=findPrice(graph, src, dst, k, dp);
